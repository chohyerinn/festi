...
Public Application Load Balancer, Apache + PHP 웹 서버, Cloud DB for MySQL/Object Storage를 분리한 3-Tier 구조로 구성했습니다. 사용자는 Load Balancer를 통해서만 접속하고, 웹 서버는 사설 네트워크로 DB와 통신합니다.
...
- Load Balancer, Web Server, Data Layer를 분리한 3-Tier 인프라 구조 설계
- Auto Scaling을 활용한 확장 가능한 웹 서버 구조 설계
...
- Presentation Tier: Public Application Load Balancer가 외부 접속과 트래픽 분산을 담당합니다.
- Application Tier: Apache + PHP 웹 서버가 사용자 기능과 DB/Object Storage 연동을 처리합니다.
- Data Tier: Cloud DB for MySQL은 private subnet에 배치하고, 게시글 이미지는 Object Storage에 분리 저장합니다.
...
이번 프로젝트에서는 외부 진입점과 트래픽 분산은 NCP Application Load Balancer가 담당하고, 웹 서버 내부 실행은 Apache가 담당하며, 데이터 저장은 Cloud DB for MySQL과 Object Storage로 분리하는 방식으로 역할을 나눴습니다.
